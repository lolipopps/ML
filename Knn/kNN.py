...
def datingClassTest():
    ...
    # 从文件中加载数据
    datingDataMat, datingLabels = file2matrix("../data/KNN/datingTestSet2.txt")  # load data setfrom file
    # 归一化数据
...
def handwritingClassTest():
    ...
    # 1. 导入数据
    hwLabels = []
    trainingFileList = os.listdir("../data/KNN/trainingDigits") # load the training set
    ...
    # 2. 导入测试数据
    testFileList = os.listdir('../data/KNN/testDigits')  # iterate through the test set
    errorCount = 0
    ...
    for i in range(mTest):
        ...
        fileStr = fileNameStr.split('.')[0]  # take off .txt
        classNumStr = int(fileStr.split('_')[0])
        vectorUnderTest = img2vector('../data/KNN/testDigits/%s' % fileNameStr)
