...
def train_naive_bayes(train_mat, train_category):
    ...
    # p0num = np.zeros(words_num)
    # p1num = np.zeros(words_num)
    p0num = np.ones(words_num)   ## 加了拉普拉斯平滑系数
    ...
    # 后面改成取 log 函数
    p1vec = np.log(p1num / p1num_all)   ##   p1num_all 所有正向词的个数 当前词的概率
...
def spam_test():
    ...
    for i in range(1, 26):
        ...
        try:
            words = text_parse(open('../data/NaiveBayes/email/spam/{}.txt'.format(i)).read())
        except:
            words = text_parse(open('../data/NaiveBayes/email/spam/{}.txt'.format(i), encoding='Windows 1252').read())
        doc_list.append(words)
        full_text.extend(words)
        class_list.append(1)
        try:
            # 添加非垃圾邮件
            words = text_parse(open('../data/NaiveBayes/email/ham/{}.txt'.format(i)).read())
        except:
            words = text_parse(open('../data/NaiveBayes/email/ham/{}.txt'.format(i), encoding='Windows 1252').read())
        doc_list.append(words)
        full_text.extend(words)
        class_list.append(0)
    # 创建词汇表
