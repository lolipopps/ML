...
def load_data():  ## 加载数据集
    train = pd.read_csv('./data/features.dat',sep=',')
    label = pd.read_csv('./data/labels.dat',)
...
## 训练模型 采用的是线性核函数
def Train():
    clf = svm.SVC(C=10.0,gamma=1, kernel='rbf',).fit(X_train, y_train)  ## 默认使用 线性核函数  训练模型 使用  X_train, y_train 数据集
    scores = cross_val_score(clf, X_test, y_test, cv=5) ## cross_val_score 指定参数来设定评测标准 默认使用KFold 或StratifiedKFold 进行数据集打乱，
    print(scores)
    ## 另一种方式实现交叉验证
